...
if ( !function_exists( 'samudra_locale_css' ) ):
    function samudra_locale_css( $uri ){
        if ( empty( $uri ) && is_rtl() && file_exists( get_template_directory() . '/rtl.css' ) )
            $uri = get_template_directory_uri() . '/rtl.css';
        return $uri;
    }
endif;
add_filter( 'locale_stylesheet_uri', 'samudra_locale_css' );
...
function lalita_theme_uri_link() {
	return 'https://wpkoi.com/samudra-wpkoi-wordpress-theme/';
}
...
if ( ! function_exists( 'samudra_remove_parent_dynamic_css' ) ) {
	add_action( 'init', 'samudra_remove_parent_dynamic_css' );
	/**
	 ...
	 */
	function samudra_remove_parent_dynamic_css() {
		remove_action( 'wp_enqueue_scripts', 'lalita_enqueue_dynamic_css', 50 );
	}
}

if ( ! function_exists( 'samudra_enqueue_parent_dynamic_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'samudra_enqueue_parent_dynamic_css', 50 );
	/**
	 * Enqueue this CSS after the child stylesheet, not after the parent stylesheet.
	 *
	 */
	function samudra_enqueue_parent_dynamic_css() {
...
if ( ! function_exists( 'samudra_customize_register' ) ) {
	add_action( 'customize_register', 'samudra_customize_register' );
	function samudra_customize_register( $wp_customize ) {
		
		// Add navigation extra button text
		$wp_customize->add_setting(
			'samudra_settings[nav_btn_text]',
			array(
				'default' => '',
				'type' => 'option',
		...
		$wp_customize->add_control(
			...
			array(
				'type' => 'text',
				'label' => __( 'Extra button text', 'samudra' ),
				'section' => 'lalita_layout_navigation',
				'settings' => 'samudra_settings[nav_btn_text]',
				'priority' => 25
			)
		);
		...
		$wp_customize->add_control(
			...
			array(
				'type' => 'text',
				'label' => __( 'Extra button URL', 'samudra' ),
				'section' => 'lalita_layout_navigation',
				'settings' => 'samudra_settings[nav_btn_url]',
				'priority' => 25
			)
		);
		
		// Rounded blog images function
		$wp_customize->add_setting(
			'samudra_settings[rounded_img]',
			array(
				'default' => false,
				'type' => 'option',
				'sanitize_callback' => 'samudra_sanitize_checkbox'
			)
		);
		...
		$wp_customize->add_control(
			...
			array(
				'type' => 'checkbox',
				'label' => __( 'Rounded blog images', 'samudra' ),
		...
		// Socials on footer
		$wp_customize->add_setting(
			'samudra_settings[socials_display_bottom]',
			array(
				'default' => false,
				'type' => 'option',
				'sanitize_callback' => 'samudra_sanitize_checkbox'
		...
		$wp_customize->add_control(
			...
			array(
				'type' => 'checkbox',
				'label' => __( 'Display on footer bar', 'samudra' ),
...
if ( ! function_exists( 'samudra_sanitize_checkbox' ) ) {
	/**
	 * Sanitize checkbox values.
	 *
	 */
	function samudra_sanitize_checkbox( $checked ) {
		return ( ( isset( $checked ) && true == $checked ) ? true : false );
	}
}

if ( ! function_exists( 'samudra_navigation_button' ) ) {
	add_filter( 'wp_nav_menu_items', 'samudra_navigation_button', 11, 2 );
	/**
	 * Add the extra button to the navigation.
	 *
	 */
	function samudra_navigation_button( $nav, $args ) {
		// Get Customizer settings
		$samudra_settings = get_option( 'samudra_settings' );
		
		// If our primary menu is set, add the extra button.
		if ( ( $args->theme_location == 'primary' ) && ( $samudra_settings['nav_btn_url'] != '' ) ) {
			return $nav . '<li class="wpkoi-nav-btn-h"><a class="wpkoi-nav-btn button" href="' . esc_url( $samudra_settings['nav_btn_url'] ) . '">' . esc_html( $samudra_settings['nav_btn_text'] ) . '</a></li>';
		}
...
if ( ! function_exists( 'samudra_body_classes' ) ) {
	add_filter( 'body_class', 'samudra_body_classes' );
	/**
	 * Adds custom classes to the array of body classes.
	 *
	 */
	function samudra_body_classes( $classes ) {
		// Get Customizer settings
		$samudra_settings = get_option( 'samudra_settings' );
		
		// Return if rounded blog images function is not exist
		if ( ! isset( $samudra_settings['rounded_img'] ) ) {
			return $classes;
		}
		
		// Rounded blog images function
		if ( $samudra_settings['rounded_img'] == true ) {
			$classes[] = 'samudra-rounded-img';
		}
...
function lalita_footer_bar() {
	// Get Customizer settings
	$samudra_settings = get_option( 'samudra_settings' );
	if ( ! isset( $samudra_settings['socials_display_bottom'] ) ) {
		$socials_display_bottom = false;
	} else {
		$socials_display_bottom = $samudra_settings['socials_display_bottom'];
	}
...
if ( ! function_exists( 'samudra_footer_classes' ) ) {
	add_filter( 'lalita_footer_class', 'samudra_footer_classes' );
	/**
	 * Adds classes to the footer if footer bar is empty, but social bar is active.
	 *
	 */
	function samudra_footer_classes( $classes ) {
		// Get Customizer settings
		$samudra_settings = get_option( 'samudra_settings' );
